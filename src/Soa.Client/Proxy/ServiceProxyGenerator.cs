...
{
    public class ServiceProxyGenerator : IServiceProxyGenerator, IDisposable, ISingletonDependency
    {
        ...
        public IEnumerable<Type> GenerateProxy(IEnumerable<Type> interfaceTypes)
        {
            ...
            var stream = CompilationUtilitys.CompileClientProxy(trees,
                assemblies.Where(c => !c.IsDynamic).Select(x => MetadataReference.CreateFromFile(x.Location))
                .Concat(new[]
                {
